...
import {BomberWomanAccountData} from '$lib/account/account-data';
import {contractNetwork} from '../networks';

const logger = logs('bomberWoman');

export const accountData = new BomberWomanAccountData();
...
// But note that BomberWoman is designed to work without external RPC
			...
			if (!signature) {
				async function signMessage() {
					const msg = stringToHex(
						'Welcome to BomberWoman, Please sign this message only on trusted frontend. This gives access to your local data that you are supposed to keep secret.',
					);
