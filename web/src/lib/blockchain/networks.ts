...
export const gameConfig: Readable<GameConfig> & {
	...
	const newValue = transformGameConfig($contractsInfos.contracts.BomberWoman.linkedData);
	gameConfig.$current = newValue;
	return newValue;
}) as Readable<GameConfig> & {
	$current: GameConfig;
};
gameConfig.$current = transformGameConfig(initialContractsInfos.contracts.BomberWoman.linkedData);
