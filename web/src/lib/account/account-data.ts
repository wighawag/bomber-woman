...
export type BomberWomanMetadata =
	| undefined
	| ({
			epoch: {
				hash: `0x${string}`;
...
export type BomberWomanTransaction = EIP1193TransactionWithMetadata<BomberWomanMetadata>;
...
function fromOnChainActionToPendingTransaction(
	hash: `0x${string}`,
	onchainAction: OnChainAction<BomberWomanMetadata>,
): PendingTransaction {
	return {
		hash,
...
export class BomberWomanAccountData extends BaseAccountHandler<AccountData, BomberWomanMetadata> {
...
function _filterOutOldActions(actions: OnChainActions<BomberWomanMetadata>): boolean {
	let changes = false;
