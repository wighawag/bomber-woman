<script lang="ts">
	...
	$: total = $bomberWomanView.tokensToCollect.reduce((p, c) => p + c.amount, 0n);

	async function claim() {
		await contracts.execute(async ({client, network: {contracts}, account, connection}) => {
			const positions: bigint[] = $bomberWomanView.tokensToCollect.map((v) => v.position);
			// TODO metadata + tracking
			const txHash = await client.wallet.writeContract({
				...contracts.BomberWoman,
...
{#if total > 0}
	<div class="panel">
		<h2 class="title">
			...
			{initialContractsInfos.contracts.BomberWoman.linkedData.currency.symbol} to claim
		</h2>
