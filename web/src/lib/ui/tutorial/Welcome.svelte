...
{:else if !$tokenClaim.inUrl && !$network.notSupported && !$account.locked}
	{#if welcomeConnected || welcomeDisconnected}
		<WelcomeContainer {onclosed}>
			<h1>Welcome!</h1>
			<p>
				BomberWoman is a permission-less and persistent game (a.k.a an "Autonomous World") where you play the role of a
				god that use any of the 5 different faction to create island on an endless sea.
			</p>
			<p>
				To do so you deposit {initialContractsInfos.contracts.BomberWoman.linkedData.currency.symbol} with the hope to get
			...
			{:else if welcomeDisconnected}
				...
				{:else}
					<p>
						BomberWoman is a fully local client and we do not run any blockchain node for it. As such, it requires a
						wallet to even read the latest game state. Please Connect to a web3 wallet.
					</p>
				{/if}
			{/if}
