		...
		if (isActionPhase) {
			if (hash) {
				...
				await fetch(`${FUZD_URI}/processQueue`)
					.then((v) => v.text())
					.then((v) => console.log(v));
				await increaseContractTime(parseInt($contractsInfos.contracts.BomberWoman.linkedData.revealPhaseDuration));
			} else {
			}
		}
	}
...
<div class="info" id="info-bar">
	...
	{:else if $connection.state !== 'Connected'}
		...
		{:else}
			<div>
				BomberWoman is a fully local client and we do not run any blockchain node for it. As such, it requires a wallet
	...
	{:else if $balance.state === 'Loaded'}
		...
		{:else if $balance.tokenBalance === 0n}
			...
			{/if}
		{:else}
			{#if $bomberWomanView.hasCommitment}
