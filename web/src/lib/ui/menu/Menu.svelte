<script lang="ts">
	...
	$: isAdmin = $account.address?.toLowerCase() === $contractsInfos.contracts.BomberWoman.linkedData.admin?.toLowerCase();

	const symbol = initialContractsInfos.contracts.BomberWoman.linkedData.currency.symbol;
	const name = initialContractsInfos.contracts.BomberWoman.linkedData.currency.name;
	$: currentReserve = $balance.reserve;
	$: currentReserveString = formatUnits(
		currentReserve,
		Number(initialContractsInfos.contracts.BomberWoman.linkedData.currency.decimals),
	);
	...
	$: allowanceString =
		allowance == 0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffn
			? 'Infinite'
			: formatUnits(allowance, Number(initialContractsInfos.contracts.BomberWoman.linkedData.currency.decimals));
	...
	function clearAllowance() {
		contracts.execute(async ({client, network: {contracts}, account}) => {
			await client.wallet.writeContract({
				...
				args: [contracts.BomberWoman.address, 0n],
