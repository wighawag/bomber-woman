...
import {prepareCommitment, zeroBytes24, zeroBytes32} from 'bomber-woman-common';
...
export async function startCommit() {
	await contracts.execute(async ({client, account, connection, network: {contracts}}) => {
		const {BomberWoman, TestTokens} = contracts;
		...
		const tokenNeeded =
			BigInt(initialContractsInfos.contracts.BomberWoman.linkedData.numTokensPerGems.slice(0, -1)) * BigInt(numMoves);
		...
		const gatheringInfoStep = {
			...
			execute: async (state: CommitState) => {
				...
				const tokenApproved = await client.public.readContract({
					...
					args: [account.address, BomberWoman.address],
				});
		...
		const txStep = {
			...
			execute: async (state: CommitState) => {
				...
				const secretSig = await localWallet.signMessage({
					message: `Commit:${network.$state.chainId}:${network.$state.contracts?.BomberWoman.address}:${epochNumber}`,
				});
				...
				// await contracts.BomberWoman.write.reveal([account.address, data.secret, moves, zeroBytes24, true, zeroAddress]);
						...
						const scheduleInfo = await fuzdData.fuzd.scheduleExecution(
							{
								...
								data,
								to: contracts.BomberWoman.address,
