...
async function deployBomberWoman(config?: {
	startTime: bigint;
	commitPeriod: bigint;
	revealPeriod: bigint;
	maxLife: number;
	...
	const BomberWoman = await fetchContract(deployments['BomberWoman'] as Deployment<typeof artifacts.IBomberWoman.abi>);
...
async function deployBomberWomanWithDefaultConfig() {
	return deployBomberWoman();
}
...
describe('TestTokens', function () {
	describe('Deployment', function () {
		...
		it('TestTokens should have the correct number of decimals', async function () {
			const {TestTokens} = await loadFixture(deployBomberWomanWithDefaultConfig);
		...
		it('TestTokens should let me approve allowances via Permit style signatures', async function () {
			const {TestTokens, TestTokensPermitSigner, tokensBeneficiary, otherAccounts} = await loadFixture(
				deployBomberWomanWithDefaultConfig,
			);
