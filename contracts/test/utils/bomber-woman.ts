...
export async function withGrid(env: GridEnv, gridString: string) {
	...
	const hash = await env.BomberWoman.write.forceSimpleCells([simpleCells], {
		account: env.bomberWomanAdmin,
	});
	// TODO wait for receipt so the test work on real networks
	if (grid.actions) {
		const config = await env.BomberWoman.read.getConfig();
		await env.Time.write.increaseTime([config.commitPhaseDuration], {account: env.bomberWomanAdmin});

		for (const action of grid.actions) {
			const player = env.otherAccounts[action.owner];
			await env.BomberWoman.write.forceMoves(
				[player, [{position: xyToBigIntID(action.x, action.y), color: action.color}]],
				{account: env.bomberWomanAdmin},
			);
		}
		await env.Time.write.increaseTime([config.revealPhaseDuration], {account: env.bomberWomanAdmin});
	}
}
...
export async function performGridActions(env: GridEnv, actionGrids: string[]) {
	...
	for (const playerIndex of Object.keys(groupedActions)) {
		...
		await env.TestTokens.write.approve([env.BomberWoman.address, amountOfTokens], {account: player});
		await env.BomberWoman.write.makeCommitmentWithExtraReserve(
	...
	for (const commitment of commitments) {
		await env.BomberWoman.write.reveal(
...
export async function getGrid(
	...
	const cellsFromContract = await env.BomberWoman.read.getCells([cellIds]);
